...
class App(tk.Tk):
    def __init__(self):
        ...
        self.mas_row = PathRow(outer, "MAS excel:", "open_file", [("Excel files", "*.xlsx *.xls"), ("All files", "*.*")])
        ...
        self.folder_row = PathRow(
            outer, "Save folder:", "folder", initial=last_folder, on_change=self._remember_folder
        )
        self.folder_row.pack(fill="x", pady=4)
        self.db_row = PathRow(outer, "SQL database:", "open_file", [("Database files", "*.db *.sqlite *.sqlite3"), ("All files", "*.*")])

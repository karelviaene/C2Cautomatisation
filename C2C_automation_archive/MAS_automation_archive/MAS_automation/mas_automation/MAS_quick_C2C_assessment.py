### Quick C2C Assessment - no mixture rules
### Standalone version of option C in MAS_automation_with_mixture_rules_current.py:
### just asks for the MAS excel, a save folder and the SQL database, then builds
### the C2C_assessment_all_scenarios excel (colour hazards pulled from the DB,
### written into the C2C assessment template). No % assessed, no mixture rules,
### no other output files. Meant for smaller projects that don't need the full
### MAS_automation_with_mixture_rules_current.py workflow.
### If MAS_automation_with_mixture_rules_current.py's shared logic changes
### (column names, template layout, DB schema), mirror the change here too -
### this file intentionally duplicates that logic rather than importing it.
...
### The C2C_assessment_all_scenarios excel is NOT built from scratch - it
### is a copy of templates/C2C_assessment_template.xlsx with the
### "detailed_overview" sheet filled in with the output of
...
### update (or re-derive) templates/C2C_assessment_template.xlsx and the
### path below, otherwise those formulas will silently read the wrong
### columns. Keep this in sync with MAS_automation_with_mixture_rules_current.py.
...
### range made Excel crash on open; now that the scan range is shrunk to
### match the project size too, this is temporarily raised to 40000 here
### (only in this quick-assessment script, not the main one) to check
### whether that fix actually solves the problem at that size - lower it
### back down (e.g. to 5000, matching MAS_automation_with_mixture_rules_current.py)
### if it still struggles.
C2C_ASSESSMENT_TEMPLATE_MAX_ROWS = 40000
...
def run_quick_c2c_assessment():
    print("--------------------------------------------------------------")
    print("Quick C2C Assessment (no mixture rules)")
    print("--------------------------------------------------------------")
    ...
    if len(all_scenarios_df) > C2C_ASSESSMENT_TEMPLATE_MAX_ROWS:
        ...
        print("This quick, mixture-rules-free assessment cannot summarise a project this size.")
        print("Please use MAS_automation_with_mixture_rules_current.py instead (option A or B).")
        print("--------------------------------------------------------------")
        return
